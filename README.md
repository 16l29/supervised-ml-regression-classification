...
| ----- | -------------------------------------------------------------------------------------------- | ------------------- |
| 1     | **Regressão**— Previsão de potência de aerogerador a partir da velocidade do vento | `aerogerador.dat` |
| 2     | **Classificação**— Reconhecimento de expressões faciais via sinais EMG             | `EMGsDataset.csv` |
...
```
.
├── 1.REGRESSAO.py          # Implementação completa da tarefa de regressão
├── 2.CLASSIFICACAO.py      # Implementação completa da tarefa de classificação
├── 3.RELATORIO.pdf        # Relatório completo com resultados e discussões
...
## 🌬️ Tópico 1 — Regressão (`1_REGRESSAO.py`)
...
## 🧠 Tópico 2 — Classificação (`2_CLASSIFICACAO.py`)
...
```bash
# Certifique-se de que aerogerador.dat está no mesmo diretório
python 1_REGRESSAO.py
```
...
```bash
# Certifique-se de que EMGsDataset.csv está no mesmo diretório
python 2_CLASSIFICACAO.py
```
...
> **Atenção:** Atualize o caminho do dataset nas linhas de leitura dos arquivos caso necessário (`np.loadtxt(...)` e `np.genfromtxt(...)`).

---

## 📄 Relatório

O arquivo [`3.RELATORIO.pdf`]() contém o relatório completo no formato de artigo científico, incluindo:
...
$$
\log P(C_k | x) \propto \log p(x | C_k) + \log \pi_k
$$

### Classificador de Friedman

$$
\Sigma_k^{(\lambda)} = (1-\lambda)\hat{\Sigma} *k + \lambda \hat{\Sigma}* {pool}
$$
